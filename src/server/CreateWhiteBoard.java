...
public class CreateWhiteBoard extends JFrame implements ChatServerListener {
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    try {
                        ...
                        user.setHost(true);
                        user.client = new StartClient().startNewClient(user.getUserName(), url);
    ...
    private CreateWhiteBoard(String s) {
    ...
    private void resizeTable(boolean bool) {
        ...
        if (n == 0) {
            if (!Arrays.asList(client).contains(String.valueOf(evt.getMessage()))) {
                ...
                JOptionPane.showMessageDialog(null,
                        evt.getMessage() + "'s request has been refused due to a duplicate name.", "Confirm", 0);
                n = 1;
            }
        } else {
            JOptionPane.showMessageDialog(null, evt.getMessage() + "'s request has been refused by the manager.",
                    "Confirm", 0);
        }
        return n;
    }
    ...
    public synchronized void serverEventQuit(ChatServerEvent evt) {
        ...
        getVect().add(list);
        ((AbstractTableModel) getJg_table().getModel()).fireTableDataChanged();
        // JOptionPane.showMessageDialog(null,evt.getMessage() + " has been removed from
        // the user list table.", "Confirm", 1);
        client = delete(getIndex(client, String.valueOf(evt.getMessage())));
        --count;
    }

    private String[] delete(int value) {
